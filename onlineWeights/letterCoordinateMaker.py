...
LAT_MIN   = -6.5
LAT_MAX   =  6.5
Z_MIN     =  0.0    # inches, relative to board bottom edge
Z_MAX     = 10.0    # inches, relative to board bottom edge (= full board height)
SNAP_GRID =  0.25
...
DRAW_W = int((LAT_MAX - LAT_MIN) * PX_PER_IN)   # 520 px  (13" × 40 px/in)
DRAW_H = int((Z_MAX   - Z_MIN)   * PX_PER_IN)   # 400 px  (10" × 40 px/in)
...
class LetterCoordinateMaker:
    ...
    def _load_existing(self):
        """If OUTPUT_FILE already exists, restore its segments so the user
        can see and continue editing the previous session's work."""
        ...
        try:
            ...
            last = segs[-1]
            if not last.get("lift_after", True):
                self.chain_tail = list(last["end"])
                self.status_var.set(
                    f"Loaded {len(segs)} segment(s).\n"
                    "Last chain still open.\n\n"
                    "Continue clicking or\npress End Chain."
                )
            else:
            ...
            self.status_var.set(f"Could not load\nprevious file:\n{exc}")
