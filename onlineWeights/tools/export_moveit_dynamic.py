...
def write_header(samples: list[Sample], output: Path, pause_dt_ms: int, corner_vel_threshold: float) -> None:
    ...
    with output.open("w", encoding="utf-8") as f:
        ...
        for s in samples:
            max_vel = max(
                abs(s.v1_rads), abs(s.v2l_rads), abs(s.v4_rads),
                abs(s.v5_rads), abs(s.v6_rads),
            )
            if s.dt_ms >= pause_dt_ms:
                pause = 1   # pen-lift / plant boundary
            elif max_vel < corner_vel_threshold:
                pause = 2   # draw-to-draw corner: zero-velocity endpoint — must stop here
            else:
                pause = 0
            f.write(
...
def main() -> int:
    ...
    p.add_argument(
        "--corner-vel-threshold", type=float, default=0.005,
        help="Waypoints where max joint velocity (rad/s) < this AND dt_ms < pause-threshold "
             "are flagged pause=2 (draw-to-draw corner: stop, no retract dwell). "
             "Default: 0.005 rad/s (effectively zero-velocity segment endpoints).",
    )
    p.add_argument(
    ...
    write_header(node.samples, output, args.pause_dt_ms_threshold, args.corner_vel_threshold)
    node.get_logger().info(
